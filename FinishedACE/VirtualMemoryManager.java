 ...
 * Standard ACE without the attempt at the modifications
 * 
 */
	...
	private static void run(){
		...
		try {
			...
			while (input.hasNextInt()) {
				...
				//get number from the input file and then extract necessary information
				...
				if(tlb.containsKey(pageNumber)){
					...
					//check the page table to see if there has been a page fault
					if (pt.checkFault(pageNumber) == true){
						...
						try {
							...
							//get the signed byte stored at the physical memory address
							memoryValue = pm.getPhysicalMemory()[pointer + offset];

							//add the size to the pointer to get the next location
							pointer += 256;

							pt.insertFrameNumber(pageNumber, ft.getFrame(nextAvailableFrame));
							//get the frame number that's just be inserted into the page table
							frameNumber = pt.getPage(pageNumber);

							//add the value into the physical memory at the correct location using the frame number
							pm.add(frameNumber, memoryValue);

							//add one so the location of the next available frame is kept
							nextAvailableFrame++;

							//had a page fault so add one to the page fault count
							pageFaults++;
							...
							//update the tlb
							if(tlb.size() == 16){
								//if tlb is full remove the first element that was inserted, use the FIFO arraylist to get that element
								tlb.remove(fifo.get(0));
								//remove that element from the FIFO arraylist also
								fifo.remove(0);
							}
							else{
								//otherwise, add page number and frame number to the TLB
								t.add(pageNumber, frameNumber);
								//keep track of the TLB using the FIFO arraylist
								fifo.add(pageNumber);
							}
						} catch (IOException e) {
					...
					//not in TLB and no page fault, so must be in page table already
					else{
				...
				output.add(number);
				output.add(physicalAddress);
				output.add((int) memoryValue);

				//calls function to print all necessary data
				printAddresses(number, physicalAddress, memoryValue);

				//add one to the number of addresses translated, to work out page fault and TLB hit rate
				addressesTranslated++;
			}
			//close the scanner
			input.close();
			...
			int correctLines = test.check(output);
			System.out.println("Number of values correctly printed " + correctLines + "/" + (addressesTranslated * 3));
			if(correctLines == (addressesTranslated * 3)){
				System.out.println("All addresses and values are a perfect match to the correct.txt file.");
			}
